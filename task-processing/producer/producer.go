...
func stringPrint(s string) *string {
	return &s
}
...
func run(log *log.Logger, cfg *config.Config, transactionsFile string) error {
	...
	screen, err := screen.NewKafkaProducerScreen(stats); if err != nil {
	...
	go func() {
        for scanner.Scan() {
            ...
            if err := producer.Produce(&kafka.Message{
                TopicPartition: kafka.TopicPartition{Topic: stringPrint(cfg.KafkaTopic), Partition: kafka.PartitionAny},
                Value:          []byte(line),
